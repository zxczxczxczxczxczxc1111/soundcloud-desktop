...
it('«Встряхнуть» не возвращает перезалив показанного или сыгранного трека', async () => {
    ...
    // В стартовую очередь идут 10 из 16 найденных вперемешку: без закреплённой случайности ни одной мелодии там нет
    // в ~0,8% прогонов (3 из 500), и тест падал на проверке, что сценарий случился (CI на dad640c)
    vi.spyOn(Math, 'random').mockReturnValue(0.5);
