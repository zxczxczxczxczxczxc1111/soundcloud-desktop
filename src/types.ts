...
export interface PlaySignal {
    ...
    /** wave:similar, wave:fresh, wave:track, wave:artist, wave:playlist, подборки wave:daily, wave:forgotten, wave:group, wave:tracks, радар wave:radar, «Моя музыка» wave:library или site:<тип очереди сайта> */
    source: string;
    ...
    genre: string;
    tags: string;
    /** Версия записи: 1 до 0.5.2, 2 с названием, адресом, обложкой и поясом, 3 с отрезками и причиной смены */
    v?: 1 | 2 | 3;
    ...
    away?: boolean;
    /** v3: реально сыгранные участки трека [с, по] в мс позиции, без перемоток; повтор участка не добавляет покрытия */
    spans?: Array<[number, number]>;
    /** v3: user, если трек сменили кнопкой, клавишей или кликом; auto, если сменил сам сайт (конец очереди, ошибка) */
    endedBy?: 'user' | 'auto';
    /** v3: трек запущен кликом по нему самому в блоке волны или панели очереди, а не переходом очереди */
    picked?: boolean;
}
