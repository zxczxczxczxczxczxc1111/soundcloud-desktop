...
<sub>Версия 0.9.0 для Windows 10 и 11 x64</sub>
...
Под волной блок с твоими лайками и плейлистами целиком. Выбираешь, что играть, и порядок: «По порядку», «Перемешать» (один артист не идёт подряд, слышанное за 3 дня пропускается) или «Умное перемешивание», где после каждых трёх своих треков идёт похожий, которого у тебя ещё нет.

### Подборки на каждый день

Ниже каждый день новая полка:
...
- **Твои вкусы**: клиент делит лайки и то, что ты дослушиваешь, по жанрам, до восьми групп, и у каждой своя волна.
...
После перезапуска клиент восстанавливает очередь, источник волны и позицию, а музыку держит на паузе, пока ты не нажмёшь play. При временном сбое сети показывает статус и повторяет подключение, сохраняя очередь. Если страница упала, зависла или открылась без сети, клиент поднимает её сам.
...
- **Строка на исполнителя**: несколько треков одного артиста собраны в одну строку. Метка альбома или «+N» раскрывает остальные, «Слушать все» ставит их следующими.
...
Бери установщик `soundcloud-desktop-0.9.0-setup-x64.exe` на странице [последнего релиза](https://github.com/zxczxczxczxczxczxc1111/soundcloud-desktop/releases/latest). Рядом лежит `SHA256SUMS`, если захочешь сверить файлы.
